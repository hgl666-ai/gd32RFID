...
{
    volatile unsigned char *p = (unsigned char *) v; while ( n-- )
        *p++ = 0;
}
...
/*
 * 3DES-ECB buffer encryption API nlen需要为8的整数倍
 */
...
/*
 * 3DES-CBC buffer encryption API nlen需要为8的整数倍
 */
...
void des_encDecTest()
{
    ...
    unsigned char	input[32] = {
        0x62, 0x51, 0x9C, 0x9B, 0xB2, 0xDC, 0xDE, 0x13,
        0xCF, 0x6F, 0x96, 0x25, 0x2F, 0xA6, 0x9A, 0x6A,
        0xD4, 0xF3, 0xEC, 0x43, 0xA8, 0x73, 0x58, 0xB5,
        0xE3, 0x17, 0x84, 0xAA, 0x5F, 0x4E, 0x43, 0xC8
    };
    ...
    /*F47FBC5EBB21BD83C143ED69F4612768566A94005DD536F45678818C724C0D9B */
    unsigned char exp_ecb_enc[32] = {
        0xF4, 0x7F, 0xBC, 0x5E, 0xBB, 0x21, 0xBD, 0x83,
        0xC1, 0x43, 0xED, 0x69, 0xF4, 0x61, 0x27, 0x68,
        0x56, 0x6A, 0x94, 0x00, 0x5D, 0xD5, 0x36, 0xF4,
        0x56, 0x78, 0x81, 0x8C, 0x72, 0x4C, 0x0D, 0x9B
    };
    printf( "DES Algorithm Test\n" );
    des3_ecb_encrypt( output,
                      input,
                      sizeof(input),
                      deskey,
                      16 );
    if ( memcmp( output, exp_ecb_enc, 32 ) == 0 )
    {
        printf( "ecb enc model passed\n" );
    }
    else
    {
        printf( "ecb enc model failed\n" );
    }
    
    des3_ecb_decrypt( output,
                      exp_ecb_enc,
                      sizeof(exp_ecb_enc),
                      deskey,
                      16 );
    if ( memcmp( output, input, 32 ) == 0 )
    {
        printf( "ecb enc model passed\n" );
    }
    else

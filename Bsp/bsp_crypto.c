                            ...
                            uint8_t       *pCipher, uint8_t *pCipher_len)
{
    ...
    if (!s_se_authenticated) {
        ...
        /* 严格模式: SE 未认证, 拒绝加密 */
        DBG_PRINTF("[SE] SE not authenticated, reject\r\n");
        return 0;
#else
        /* 透传模式: 未认证时明文直出 (开发调试) */
    ...
    if (sw != 0x9000) {
        DBG_PRINTF("[SE] write_se_data failed, SW=0x%04X\r\n", sw);
        return 0;
    }
    ...
    if (sw != 0x9000) {
        DBG_PRINTF("[SE] get_se_data failed, SW=0x%04X\r\n", sw);
        return 0;
    }
